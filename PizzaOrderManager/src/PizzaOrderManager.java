...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            sender = Integer.parseInt(value);
            priceMap.add(key, sender);
        }
        in.close();
    }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (!current.equals("")) {
            if (sizePriceMap.hasKey(current)) {
                for (Map.Pair<String, Integer> size : sizePriceMap) {
                    if (size.key().equals(current)) {
                        total = total + size.value();
                    }
                }
            }
            if (toppingPriceMap.hasKey(current)) {
                for (Map.Pair<String, Integer> topping : toppingPriceMap) {
                    if (topping.key().equals(current)) {
                        total = total + topping.value();
                    }
                }
            }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        int pennies = change % dollarValue, dollarAmount = change / dollarValue;
        output.println("Price: " + dollarAmount + "." + pennies);
    }
