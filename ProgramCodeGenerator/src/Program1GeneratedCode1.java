        ...
        switch (s.kind()) {
            ...
            case CALL: {
                ...
                if (context.hasKey(call)) {
                    Map.Pair<String, Statement> temp = context.remove(call);
                    generateCodeForStatement(temp.value(), context, cp);
                } else if (call.equals("move")) {
                    cp.add(cp.length(), 0);
                } else if (call.equals("turnleft")) {
                    cp.add(cp.length(), 1);
                } else if (call.equals("turnright")) {
                    cp.add(cp.length(), 2);
                } else if (call.equals("infect")) {
                    cp.add(cp.length(), 3);
                } else if (call.equals("skip")) {
                    cp.add(cp.length(), 4);
                } else {
                    Reporter.assertElseFatalError(false, call)
                }
            }
            default: {
